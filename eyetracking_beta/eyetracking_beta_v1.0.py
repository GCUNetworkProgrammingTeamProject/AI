...
concentration = 0
frame_count = 0
def calculate_concentration(_concentrate_eye, _concentrate_head):
    global concentration
    # 집중도 계산 방식을 원하는 대로 정의하세요.
    # 이 예제에서는 눈동자 비율과 고개 방향 비율을 조합하여 집중도를 계산합니다.
    # 이 값은 -1에서 1 사이로 정규화됩니다.
    print(f"{_concentrate_eye}, {_concentrate_head}")
    if (not _concentrate_eye and not _concentrate_head):
        return 1
    else:
        return 0
...
def warn_eye_direction(_criteria_finished, _gaze_ratio, _eye_direction_criteria, _margin_eye):
    ...
    if _criteria_finished and _gaze_ratio < _eye_direction_criteria - _margin_eye:
        """if not is_time_counting_eye:
            ...
            cause = 2
        print("눈동자 왼쪽으로 벗어남")
        #print(f"_gaze_ratio : {_gaze_ratio}")"""
...
def main():
    ...
    global num_frames, frame_count, concentration_score, wrong_eye_direction, wrong_head_direction
    ...
    while True:
        ...
        if frame_count == 30:
            ...
            frame_count = 0
            print(f"concentration_score : {concentration_score}")
            concentration_score = 0

        if faces:
            concentration_score += 1/30
        else:
            concentration_score -= 1/30
        for face in faces:
            #print("실행됨")
